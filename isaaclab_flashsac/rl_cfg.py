...
class FlashSACAlgorithmCfg:
    ...
    replay_buffer_size: int = 10_000_000
...
G1_TRACKING_SYMMETRY_CFG: dict = {
    "use_data_augmentation": True,
    "use_mirror_loss": False,
    "data_augmentation_func": "isaaclab_flashsac.mdp.obs.symmetry.g1.wbt:compute_symmetric_states",
    "mirror_loss_coeff": 0.0,
}
"""G1 motion-tracking left-right mirror data augmentation (opt-in via ``flashsac-train --symmetry``).

Off by default: whole-body tracking is normally trained without symmetry augmentation.
"""
...
@register_task
@configclass
class AnymalCRoughFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Rough-Anymal-C-v0"


@register_task
@configclass
class AnymalCFlatFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Flat-Anymal-C-v0"


@register_task
@configclass
class AnymalDRoughFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Rough-Anymal-D-v0"


@register_task
@configclass
class AnymalDFlatFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Flat-Anymal-D-v0"


@register_task
@configclass
class G1RoughFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Rough-G1-v0"


@register_task
@configclass
class G1FlatFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Flat-G1-v0"


@register_task
@configclass
class H1RoughFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Rough-H1-v0"


@register_task
@configclass
class H1FlatFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Velocity-Flat-H1-v0"
...
###############################
# BeyondMimic-style whole-body motion tracking. The env exposes an asymmetric observation
# split via the "policy"/"critic" groups.


@configclass
class G1TrackingAlgorithmCfg(FlashSACAlgorithmCfg):
    """FlashSACAlgorithmCfg for the G1 tracking tasks.

    mini_batch_size 8192: on single-GPU dance1_subject2 (100k iters, 1024 envs) it evaluates at
    63.3 avg return vs 60.7 for the 2048 default, closing most of the gap to the 4-GPU run (65.0).
    """
...
@register_task
@configclass
class G1FlatTrackingFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Tracking-Flat-G1-v0"
    obs_groups: dict = field(default_factory=lambda: {"actor": ["policy"], "critic": ["critic"]})
    action_bound: str = "scalar"
    action_bound_scale: float = 3.0
    max_iterations: int = 100_000
    algorithm: G1TrackingAlgorithmCfg = field(default_factory=G1TrackingAlgorithmCfg)


@register_task
@configclass
class G1FlatTrackingWoSEFlashSACCfg(G1FlatTrackingFlashSACCfg):
    task_name: str = "Isaac-Tracking-Flat-G1-WoSE-v0"
...
@register_task
@configclass
class ShadowReposeCubeFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Repose-Cube-Shadow-Direct-v0"


@register_task
@configclass
class AllegroReposeCubeFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Repose-Cube-Allegro-Direct-v0"
...
@register_task
@configclass
class FrankaOpenDrawerFlashSACCfg(FlashSACRunnerCfg):
    task_name: str = "Isaac-Open-Drawer-Franka-v0"
    action_bound_scale: float = 3.0


@register_task
@configclass
class FrankaReachFlashSACCfg(FlashSACRunnerCfg):
